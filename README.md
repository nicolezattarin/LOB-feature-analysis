...
* **Zatta 29/12:** the file `book_loader.py` is meant to be run on the VM, see its documentation in the file itself. The idea is to to create df from book objects and to save data in .csv files, in such a way that we can work on our own devices without using the VM. Data cannot be uploaded on github, thus if you create files with a large amount of data upload them [here](https://drive.google.com/drive/folders/1LP0KT5O1YQT1Vf3692nPeoT5SCsrJtUk?usp=sharing). 
